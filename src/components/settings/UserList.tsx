...
import { User, MoreHorizontal, Edit, Trash2, ShieldCheck, Building2, ShieldAlert, Lock } from "lucide-react";
...
export const UserList: React.FC<UserListProps> = ({ users, onEdit, onDelete, canManage = false }) => {
  ...
  return (
    <div className="space-y-3">
      {users.map((user) => {
        // Déterminer le rang de l'utilisateur dans la liste
        const userRole = roles.find(r => r.name === user.poste) || { rank: user.isSuperAdmin ? 100 : 0 };
        const userRank = user.isSuperAdmin ? 100 : userRole.rank;
        
        // Un utilisateur ne peut gérer que ceux de rang inférieur ou égal
        // Et seul un Super Admin peut gérer un autre Super Admin (ou soi-même)
        const canEditThisUser = canManage && (currentUserRank >= userRank || currentUser.isSuperAdmin);
