...
import { Plus, FileText, Trash2, Edit, ShoppingCart, BarChart3, Receipt, History } from "lucide-react";
...
export const ProjectDetail: React.FC<ProjectDetailProps> = ({ project, isOpen, onClose }) => {
  ...
  const [technicalEntries, setTechnicalEntries] = useState<any[]>([]);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [invData, purData, techData] = await Promise.all([
        fetcher(`/projects/${project.id}/sales-invoices`),
        fetcher(`/purchases?projet_id=${project.id}`),
        fetcher(`/projects/${project.id}/technical-entries`)
      ]);
      setInvoices(invData);
      setPurchases(purData);
      setTechnicalEntries(techData);
    } catch (err) {
      // Mock data pour la démo
      setInvoices([
        { id: 1, numero_facture: "FV-2026-001", date_facture: "2026-01-10", montant_ht: 5000, tva_pct: 19, statut: "Payé", note: "Acompte 10%" },
        { id: 2, numero_facture: "FV-2026-002", date_facture: "2026-02-15", montant_ht: 15000, tva_pct: 19, statut: "Payé", note: "Situation n°1" },
      ]);
      setPurchases([
        { id: 101, fournisseur: "Fournisseur X", numero_facture: "FA-998", date_facture: "2026-02-10", montant_ht: 1200, categorie: "Matériel" },
      ]);
      setTechnicalEntries([
        { id: 1, type: "Réunion", date: "2026-01-05", libelle: "Réunion de démarrage", effectue_par: "Ing. Ahmed" },
        { id: 2, type: "Relevée", date: "2026-01-20", libelle: "Relevé topographique", effectue_par: "Technicien Ali" },
        { id: 3, type: "Envoyé", date: "2026-02-01", libelle: "Envoi Dossier APD", effectue_par: "Ing. Ahmed" },
      ]);
    } finally {
      setLoading(false);
    }
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="sm:max-w-[850px] overflow-y-auto border-l-0 shadow-2xl">
        ...
        <Tabs defaultValue="timeline" className="w-full">
          <TabsList className="grid w-full grid-cols-5 mb-8 bg-slate-100/50 p-1.5 rounded-2xl">
            <TabsTrigger value="timeline" className="rounded-xl data-[state=active]:shadow-md gap-2">
          ...
          <TabsContent value="timeline" className="py-4">
            <ProjectTimeline invoices={invoices} technicalEntries={technicalEntries} />
          </TabsContent>
                    ...
                    <TableRow key={inv.id} className="group">
                      ...
                      <TableCell>
                        <Badge variant="outline" className={cn(
                          ...
                          inv.statut === "Payé" ? "border-emerald-200 text-emerald-600 bg-emerald-50" : "border-slate-200 text-slate-500"
                        )}>
                          {inv.statut}
                        </Badge>
                      </TableCell>
